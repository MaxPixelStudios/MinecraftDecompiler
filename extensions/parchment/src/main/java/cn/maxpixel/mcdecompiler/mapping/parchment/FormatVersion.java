...
public record FormatVersion(int major, int minor, int patch) {
    ...
    public FormatVersion {
        ...
        if (minor < 0) throw new IllegalArgumentException("Minor version " + minor + "must not be negative");
        if (patch < 0) throw new IllegalArgumentException("Patch version " + patch + "must not be negative");
    }
