...
public record MixinTargetSelector(@Nullable Type owner, @Nullable String name, @Nullable String quantifier, @Nullable String descriptor, boolean field) {
    ...
    public MixinTargetSelector remap(MappingRemapper remapper, String mappedName) {
        return new MixinTargetSelector(owner == null ? null : Type.getType(remapper.mapDesc(owner.getDescriptor())), mappedName, quantifier,
                descriptor == null ? null : (field ? remapper.mapDesc(descriptor) : remapper.mapMethodDesc(descriptor)), field);
    }
